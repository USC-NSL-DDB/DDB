from typing import Dict, List, Optional, Set, TYPE_CHECKING, Union, Any
...
if TYPE_CHECKING:
    from gdb.FrameDecorator import ( # type: ignore
        FrameDecorator as _FrameDecorator,
        DAPFrameDecorator as _DAPFrameDecorator,
    )  # type: ignore
...
class FilterRule:
    ...
    def to_dict(self) -> Dict[str, str]:
        ...
        return {"pattern": self.pattern, "match_type": self.match_type.value}
...
class FilterConfig:
    ...
    def add_function_rule(self, pattern: str, match_type: MatchType):
        ...
        # Avoid duplicates
        if not any(
            r.pattern == pattern and r.match_type == match_type
    ...
    def add_file_rule(self, pattern: str, match_type: MatchType):
        ...
        # Avoid duplicates
        if not any(
            r.pattern == pattern and r.match_type == match_type for r in self.file_rules
        ):
    ...
    def should_filter_frame(
        self, func_name: Optional[str], file_name: Optional[str]
    ) -> bool:
        """
        ...
        if func_name:
            for rule in self.function_rules:
                if rule.matches(func_name):
                    ...
                    dbg(
                        LogLevel.TRACE,
                        f"Function '{func_name}' matched rule: {rule.pattern}",
                    )
                    break
        ...
        if not matched and file_name:
            for rule in self.file_rules:
                if rule.matches(file_name):
                    ...
                    dbg(
                        LogLevel.TRACE,
                        f"File '{file_name}' matched rule: {rule.pattern}",
                    )
                    break
    ...
    def filter(self, frame_iter):
        ...
        for frame in frame_iter:
            ...
            if should_hide:
                dbg(
                    LogLevel.DEBUG,
                    f"Filtering out frame: func={func_name}, file={file_name}",
                )
        ...
        try:
            if cmd == "enable":
                ...
                if len(args) < 2:
                    ...
                    gdb.write(
                        "Usage: ddb-filter add-function <pattern> [--type exact|glob|regex]\n"
                    )
                    return
                pattern = args[1]
                ...
                if len(args) < 2:
                    gdb.write("Error: Missing pattern\n")
                    gdb.write(
                        "Usage: ddb-filter add-file <pattern> [--type exact|glob|regex]\n"
                    )
                    return
                pattern = args[1]
                match_type = self._parse_match_type(args)
    ...
    def _handle_preset(self, args: List[str]):
        ...
        if subcmd == "list":
            ...
            for name, preset in FRAMEWORK_PRESETS.items():
                status = (
                    " (active)" if name in self.filter.config.active_presets else ""
                )
...
class DDBFilterMICommand(gdb.MICommand):
            ...
            if action == "--enable":
                ...
                if mode in ["blacklist", "whitelist"]:
                    ...
                    return self._error_response(
                        "Invalid mode (must be blacklist or whitelist)"
                    )
    ...
    def _status_response(self) -> Dict[str, object]:
        ...
        return {"message": "success", "config": self.filter.config.to_dict()}
