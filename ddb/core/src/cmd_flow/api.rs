...
//! ```
//!
//! ### Command with custom formatter (wait for result)
//! ```no_run
//! # use core::cmd_flow::api;
//! # async fn example() -> Result<(), api::Error> {
//! let result = api::send_and_return("-thread-info")?
...
//! ```
//!
//! ### Alternative: intercept (equivalent to send_and_return().with())
//! ```no_run
//! # use core::cmd_flow::api;
//! # async fn example() -> Result<(), api::Error> {
//! let result = api::intercept("-thread-info")?
//!     .with(api::ThreadInfoFormatter)
//!     .to(api::Target::Broadcast)
//!     .await?;
//! # Ok(())
...
impl SendAndReturnBuilder {
    /// Specify a custom formatter for this command
    pub fn with<F: DynFormatter + 'static>(self, formatter: F) -> SendAndReturnWithFormatterBuilder<F> {
        SendAndReturnWithFormatterBuilder {
            parsed_cmd: self.parsed_cmd,
            formatter,
        }
    }
...
/// Builder for sending a command with custom formatter and waiting for result
pub struct SendAndReturnWithFormatterBuilder<F: DynFormatter + 'static> {
    parsed_cmd: ParsedInputCmd,
    formatter: F,
}

impl<F: DynFormatter + 'static> SendAndReturnWithFormatterBuilder<F> {
    /// Route the command to the specified target with custom formatter and wait for result
    pub async fn to(self, target: Target) -> Result<FinishedCmd, Error> {
        let (cmd_to_send, _) = prepare_to_send(self.parsed_cmd, self.formatter)?;
        let result = get_router().send_to_ret(target, cmd_to_send).await?;
        Ok(result)
    ...
    /// Route the command to the specified target or default target and wait for result
    /// If the target is not specified, it defaults to the target specified in the original command.
    pub async fn to_or_default(self, target: Option<Target>) -> Result<FinishedCmd, Error> {
        if let Some(target) = target {
            self.to(target).await
    ...
    /// Route the command to the target specified in the command itself and wait for result
    /// If the command does not specify a target, it defaults to the default target.
    pub async fn to_default_target(self) -> Result<FinishedCmd, Error> {
        let (cmd_to_send, target) = prepare_to_send(self.parsed_cmd, self.formatter)?;
        let result = get_router().send_to_ret(target, cmd_to_send).await?;
...
/// Builder for intercepting command output with custom formatters (waits for result)
...
/// Builder that allows setting a custom formatter for intercept
pub struct InterceptFormatterBuilder {
    parsed_cmd: ParsedInputCmd,
}

impl InterceptFormatterBuilder {
    /// Specify the formatter to use for this command and wait for result
...
mod tests {
    ...
    fn test_intercept_builder_construction() {
        let builder = intercept("-thread-info").unwrap();
        assert_eq!(builder.parsed_cmd.prefix, "-thread-info");
        assert_eq!(builder.parsed_cmd.args, "");
        assert_eq!(builder.parsed_cmd.external_token, None);
        assert_eq!(builder.parsed_cmd.target, Target::default());
    ...
    #[test]
    fn test_send_and_return_with_custom_formatter() {
        // Test that send_and_return().with() can be constructed with a custom formatter
