...
log "running cloudlab_profile setup.sh (best effort)"
wget -qO- "$SETUP_URL" | bash || log "setup.sh exited non-zero — repairing below"
...
# ─── 4. python kubernetes client (needed by setup_ddb.sh) ────────────────────
