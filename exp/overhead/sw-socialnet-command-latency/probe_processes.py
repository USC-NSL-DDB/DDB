...
def main() -> None:
    ...
    for pod in pods:
        ...
        debuggers = [
            ...
            if container.get("name", "").startswith(args.debugger_prefix)
            and statuses.get(container["name"], {}).get("state", {}).get("running") is not None
        ]
        if len(debuggers) != 1:
            raise SystemExit(
                f"{name}: expected one running {args.debugger_prefix}* debugger sidecar, "
                f"found {len(debuggers)}"
            )
        output = subprocess.check_output(
            base + ["exec", name, "-c", debuggers[0], "--", "sh", "-lc", PROBE],
            text=True,
        ).strip()
